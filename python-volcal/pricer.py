...
def DoubleBGCharFunc(p, Ap1, Am1, Lp1, Lm1, Ap2, Am2, Lp2, Lm2, riskFreeRate=0, curry=False):
    # Characteristic function for Double BG model
    # Levy measure k(x) = Am*exp(Lm*x)/|x| for x<0, Ap*exp(-Ap*x)/x for x>0
    K1 = np.nan_to_num(Ap1*np.log(Lp1/(Lp1-1))+Am1*np.log(Lm1/(Lm1+1)))
    K2 = np.nan_to_num(Ap2*np.log(Lp2/(Lp2-1))+Am2*np.log(Lm2/(Lm2+1)))
    if curry:
        def charFunc(u):
            ...
            chExp2 = Ap2*np.log(Lp2/(Lp2-1j*u))+Am2*np.log(Lm2/(Lm2+1j*u))-1j*u*K2
            def charFuncFixedU(u, maturity): # u is dummy
                return np.exp((iur+p*chExp1+(1-p)*chExp2)*maturity)
            return charFuncFixedU
    else:
        def charFunc(u, maturity):
            return np.exp((1j*u*riskFreeRate+p*(Ap1*np.log(Lp1/(Lp1-1j*u))+Am1*np.log(Lm1/(Lm1+1j*u))-1j*u*K1)+(1-p)*(Ap2*np.log(Lp2/(Lp2-1j*u))+Am2*np.log(Lm2/(Lm2+1j*u))-1j*u*K2))*maturity)
    return charFunc
